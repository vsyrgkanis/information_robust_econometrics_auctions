...
def main(args):
    ...
    # Re-construct the (mu,sigma) grid
    ...
    # Three dimensional matrix that holds the quantiles for each subsample, mu, sigma triplet
    sple_mat = np.zeros((opts.nb_sples, len(mu_vect), len(sigma_vect)))
    for sple in range(0, opts.nb_sples):
        result_list = []
        for sweep_it in range(0, opts.sweep_num):
            sweep_np = np.load("{}_{:03d}_sple_{}.npy".format(
                opts.out_file, sweep_it + 1, sple))
        ...
        sple_mat[sple, :, :] = lower_bound_mat

    np.save("{}_subsampling.npy".format(opts.out_file), sple_mat)
