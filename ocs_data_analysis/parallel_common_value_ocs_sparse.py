...
TODO
- Integrate with parse_file.py functions that Juba implemented
"""
import openpyxl
import numpy as np
# import random
...
def main(args):
    ...
    parser.add_argument("--out", dest="out_file",
                        type=str, help='Output file for sharp set matrix', default="sharp-set_001.npy")
    ...
    print("Output file: {}".format(opts.out_file))
    ...
    result_list = Parallel(n_jobs=num_workers, verbose=6)(delayed(run_parametric_lp)(
        mu, sigma, max_value, nb_bidders, bid_pdf) for (mu, sigma) in sweep_params)

    print('Total time to run all params: {}s'.format(time.time() - t))

    np.save(opts.out_file, np.asarray(result_list))
